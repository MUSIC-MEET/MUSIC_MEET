# 업로드 글 목록 조회.md
### API명 : `/cover/list?page={page}?type={type}?search={search}`

### method : GET

### comment : 회원 개별 업로드 글 목록을 페이지 단위로 반환해주는 API
###           1페이지는 10개임
###           page : 호출할 페이지 번호, type : latest or popular, search : 검색할 단어
###           type과 search는 생략가능함 생략시 최신순으로 정렬됨
###           해당 예시는 page=1을 입력한 값임
